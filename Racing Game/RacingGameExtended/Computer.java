...
class Computer implements Serializable{
	...
	private static final long serialVersionUID = -1011841902920137091L;
	...
	private boolean QuarterFlag = false;
	private boolean HalfFlag = false;
	private boolean ThreeQuarterFlag = false;
	private ArrayList<Double> checkPoint;
	...
	public Computer(int length) {
		...
		endPos = length - 1;
		map = myMap.getMap();
		checkPoint = new ArrayList<Double>();
		for (int i = 1; i <= 3; i++) {
			checkPoint.add(0.25 * i * this.length);
		}
	}
	
	@Override
	public boolean equals(Object other) {
		Computer temp = (Computer) other;
		if (temp!= null && this.getClass() == temp.getClass()) {
			if (	this.map.equals(temp.map) && 
					this.length == temp.length && 
					this.mobTotal == temp.mobTotal && 
					this.username.equals(temp.username) &&
					this.curPos == temp.curPos &&
					this.endPos == temp.endPos &&
					this.snakeBites == temp.snakeBites &&
					this.vultureBites == temp.vultureBites &&
					this.cricketBites == temp.cricketBites &&
					this.trampolineJumps == temp.trampolineJumps &&
					this.rollCounter == temp.rollCounter &&
					this.QuarterFlag == temp.QuarterFlag &&
					this.HalfFlag == temp.HalfFlag &&
					this.ThreeQuarterFlag == temp.ThreeQuarterFlag &&
					this.checkPoint.equals(temp.checkPoint))
			{
				return true;
			}
			else {
	...
	public String toString() {
		return this.username + " is at position " + this.curPos + " after " + this.rollCounter + " rolls"; 
	}
	...
	public void decrementCurrentPosition(int z,Scanner s) 
			throws SaveGameException,GameWinnerException{
		curPos -= z;
		int flag = 0;
		if (curPos < 0) {
			flag = 1;
	...
	public void saveMessage(Scanner s)
	...
	public void resumeGame(Scanner s)
			throws GameWinnerException,SaveGameException{
		while (true) {
			if (curPos == endPos) {
				throw new GameWinnerException();
			}
			...
			if (curPos == endPos) {
				...
				try {
					...
					snakeBites++;
					decrementCurrentPosition(posData.get(0),s);
				} catch (VultureBiteException vb) {
					vultureBites++;
					decrementCurrentPosition(posData.get(1),s);
				} catch (CricketBiteException cb) {
					cricketBites++;
					decrementCurrentPosition(posData.get(2),s);
...
//					System.out.println(e.getMessage());
			...
			if (curPos >= checkPoint.get(2) && !ThreeQuarterFlag) {
				ThreeQuarterFlag = true;
				this.saveMessage(s);
			}
			else if (curPos >= checkPoint.get(1) && !HalfFlag && !ThreeQuarterFlag) {
				HalfFlag = true;
				this.saveMessage(s);
			}
			else if (curPos  >= checkPoint.get(0) && !QuarterFlag && !HalfFlag && !ThreeQuarterFlag) {
				QuarterFlag = true;
				this.saveMessage(s);
			}
		}
...
class Map implements Serializable{
	...
	private static final long serialVersionUID = -1122769583324295722L;
	private ArrayList<Integer> mobCounter;
	private ArrayList<Tile> map;
	private ArrayList<Integer> posData;
...
class Dice implements Serializable{
	private static final long serialVersionUID = 739164803827335257L;
	private Random rand;
